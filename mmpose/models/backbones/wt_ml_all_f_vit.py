...
class WTM_A_ML(nn.Module):
    def __init__(self, in_dim, dim, out_dim, kernel_size, num_heads, dilations,  size):
        ...
        self.last_conv = nn.Sequential(nn.Conv2d(2*dim, dim, kernel_size=1, stride=1),
                                       nn.BatchNorm2d(dim),
    ...
    def forward(self, inputs):
        ...
        w_ = torch.cat([w0, inputs[4]], dim=1)
...
class Connect(nn.Module):
    def __init__(self, in_channels=768, out_channels=128, ratio=1):
        ...
        self.conn_5 = nn.Sequential(
            nn.ConvTranspose2d(768, hidden_features, kernel_size=2, stride=2, padding=0),
            nn.BatchNorm2d(hidden_features),
            nn.ReLU(),
            nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(hidden_features),
...
class WT_ML_ALL_F_ViTV2(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(WT_ML_ALL_F_ViTV2, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(WT_ML_ALL_F_ViTV2, self).init_weights()
