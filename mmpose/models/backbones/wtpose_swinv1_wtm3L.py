...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  4 09:21:28 2024
...
class WTM3D_L(nn.Module):
    ...
    def __init__(self, in_dim, dim, out_dim, window_size, num_heads, dilations, stride, size, low_level_dim, reduction):
        ...
        self.conv2 = nn.Conv2d(2*dim, dim, 1, bias=False)
    ...
    def forward(self, inputs, low_level_feat):
        ...
        w1 = torch.reshape(inputs[0], (-1, inputs[0].shape[1]//self.dim, inputs[0].shape[2], inputs[0].shape[3], self.dim))
        w2 = torch.reshape(inputs[1], (-1, inputs[1].shape[1]//self.dim, inputs[1].shape[2], inputs[1].shape[3], self.dim))
        w3 = torch.reshape(inputs[2], (-1, inputs[2].shape[1]//self.dim, inputs[2].shape[2], inputs[2].shape[3], self.dim))
        w4 = torch.reshape(inputs[3], (-1, inputs[3].shape[1]//self.dim, inputs[3].shape[2], inputs[3].shape[3], self.dim))
        ...
        # print(f'22:{w2.size()}')
        # print(f'4:{w4.size()}')
        # w4 = F.interpolate(w4, size=(inputs[2].shape[1]//self.dim, inputs[2].shape[2], inputs[2].shape[3]), mode='trilinear', align_corners=True)
        w4 = w4.permute(0, 4, 1,2,3)
        w4 = F.interpolate(w4, size=(4, 16, 12), mode='trilinear', align_corners=True)
        w4 = w4.permute(0, 2, 3, 4,1)
        # print(f'4:{w4.size()}')
        
        w3 = torch.cat([w4, w3], dim=1)
        w3 = self.wtb3_0(w3)
        w3 = self.wtb3_1(w3)
        w3 = w3.permute(0, 4, 1,2,3)
        # w3 = F.interpolate(w3, size=(inputs[1].shape[1]//self.dim, inputs[1].shape[2], inputs[1].shape[3]), mode='trilinear', align_corners=True) 
        w3 = F.interpolate(w3, size=(2, 32, 24 ), mode='trilinear', align_corners=True) 
        w3 = w3.permute(0, 2, 3, 4,1)
        # print(f'3:{w3.size()}')
        
        w2 = torch.cat([w3, w2], dim=1)
        w2 = self.wtb2_0(w2)
        w2 = self.wtb2_1(w2)
        w2 = w2.permute(0, 4, 1,2,3)
        # w2 = F.interpolate(w2, size=(inputs[0].shape[1]//self.dim, inputs[0].shape[2], inputs[0].shape[3]), mode='trilinear', align_corners=True) 
        w2 = F.interpolate(w2, size=(1, 64, 48), mode='trilinear', align_corners=True)
        w2 = w2.permute(0, 2, 3, 4,1)
        # print(f'2:{w2.size()}')
         
        w1 = torch.cat([w2, w1], dim=1)       
        w1 = self.wtb1_0(w1)
        w1 = self.wtb1_1(w1)
        # print(f'1:{w1.size()}')
        w1 = w1.permute(0,2,3,4,1)
        w1 = w1.reshape(-1, w1.shape[1], w1.shape[2], w1.shape[3]*w1.shape[4])
...
class wtposeSwinV1WTM3L(BaseBackbone):
    """ Swin Transformer
    ...
    def __init__(self,
        ...
        super(wtposeSwinV1WTM3L, self).__init__(init_cfg=init_cfg)
    ...
    def train(self, mode=True):
        ...
        super(wtposeSwinV1WTM3L, self).train(mode)
        self._freeze_stages()
