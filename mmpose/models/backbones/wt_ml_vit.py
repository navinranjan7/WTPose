...
class Connect(nn.Module):
    def __init__(self, in_channels=768, out_channels=128, ratio=1):
        ...
        # Define shared hidden features
        hidden_features = self.in_channels // self.ratio

        # Define layers for different upsampling and downsampling steps
    ...
    def forward(self, inputs):
        """Forward pass for connect logic."""
        c1 = self.conv_transpose_1(inputs[0])
        c1 = self.conv_transpose_2(c1)
        # print(f'c1_: {c1.size()}')

        c2 = self.conv_transpose_3(inputs[1])
        # print(f'c2_: {c2.size()}')

        c3 = self.conv1x1(inputs[2])
        # print(f'c3_: {c3.size()}')

        c4 = self.pool(inputs[3])
        c4 = self.conv_pool_1(c4)
        # print(f'c4_: {c4.size()}')
        return [c1, c2, c3, c4]
