...
class WTM_ML(nn.Module):
    def __init__(self, in_dim, dim, out_dim, kernel_size, num_heads, dilations, size, low_level_dim, reduction):
        ...
        self.wtm3_0 = NATLayer(2*dim, num_heads = num_heads[3], kernel_size=kernel_size[1], dilation = dilations[0])
        self.wtm3_1 = NATLayer(2*dim, num_heads = num_heads[3], kernel_size=kernel_size[1], dilation = dilations[1])
        self.wtm2_0 = NATLayer(2*dim, num_heads = num_heads[2], kernel_size=kernel_size[1], dilation = dilations[2])
        self.wtm2_1 = NATLayer(2*dim, num_heads = num_heads[2], kernel_size=kernel_size[1], dilation = dilations[3])
        self.wtm1_0 = NATLayer(2*dim, num_heads = num_heads[1], kernel_size=kernel_size[1], dilation = dilations[4])
        self.wtm1_1 = NATLayer(2*dim, num_heads = num_heads[1], kernel_size=kernel_size[1], dilation = dilations[5])
        self.wtm0_0 = NATLayer(dim, num_heads = num_heads[0], kernel_size=kernel_size[1], dilation = dilations[6])
        self.wtm0_1 = NATLayer(dim, num_heads = num_heads[0], kernel_size=kernel_size[1], dilation = dilations[7])
...
class WTPose_MLv1(BaseBackbone):
    """ Swin Transformer
    ...
    def __init__(self,
        ...
        super(WTPose_MLv1, self).__init__(init_cfg=init_cfg)
    ...
    def train(self, mode=True):
        ...
        super(WTPose_MLv1, self).train(mode)
        self._freeze_stages()
    ...
    def init_weights(self, pretrained=None):
        ...
        if (isinstance(self.init_cfg, dict)
            ...
            super(WTPose_MLv1, self).init_weights()
            if self.use_abs_pos_embed:
