...
class WTM_A_ML(nn.Module):
    def __init__(self, in_dim, dim, out_dim, kernel_size, num_heads, dilations,  size):
        ...
        self.wtm3_0 = NATLayer(8*dim, num_heads = num_heads[0], kernel_size=kernel_size, dilation = dilations[0])
        self.wtm3_1 = NATLayer(8*dim, num_heads = num_heads[0], kernel_size=kernel_size, dilation = dilations[1])
        self.wtm2_0 = NATLayer(4*dim, num_heads = num_heads[1], kernel_size=kernel_size, dilation = dilations[2])
        self.wtm2_1 = NATLayer(4*dim, num_heads = num_heads[1], kernel_size=kernel_size, dilation = dilations[3])
        self.wtm1_0 = NATLayer(2*dim, num_heads = num_heads[2], kernel_size=kernel_size, dilation = dilations[4])
        self.wtm1_1 = NATLayer(2*dim, num_heads = num_heads[2], kernel_size=kernel_size, dilation = dilations[5])
        ...
        self.conv_2_1x1 = nn.Sequential(nn.Conv2d(8*dim, 4*dim, 1),
                                       nn.BatchNorm2d(4*dim))
        self.conv_2_up = nn.Sequential(nn.ConvTranspose2d(4*dim, 2*dim, kernel_size=2, stride=2),
    ...
    def forward(self, inputs):
        ...
        # x = torch.cat([w0, low_level_feat], dim=1)
...
class UpSample(nn.Module):
    def __init__(self, in_channels=768, out_channels=128, ratio=1, up_step=0):
    ...
    #             x = self.conv_2(x)
    #     elif isinstance(self.up_step, int) and self.up_step == 0:
    #         x = self.conv1x1(x)
    #     elif self.up_step =="downsample":
    #         x = self.pool(x)
    #         x = self.conv_1(x)
    #     return x
    def forward(self, x):
        print(f'x:{x.size()}')
        if isinstance(self.up_step, int) and self.up_step > 0:
            ...
            raise ValueError(f"Invalid value for up_step: {self.up_step}")
        return x
...
#         # Define upsampling and downsampling operations
#         self.upsample_layers = self._build_upsample_layers(hidden_features)
#         self.downsample_layer = self._build_downsample_layer(hidden_features)
#         self.conv1x1 = nn.Conv2d(self.in_channels, self.out_channels, kernel_size=1, stride=1)

#     def forward(self, x):
#         if isinstance(self.up_step, int) and self.up_step > 0:
#             for _ in range(self.up_step):
#                 x = self.upsample_layers(x)  # Transpose layer and Conv Layers
#                 # x = self.upsample_layers[1](x)  # Conv layer
#         elif isinstance(self.up_step, int) and self.up_step == 0:
#             x = self.conv1x1(x)
#         elif self.up_step == "downsample":
...
#     def _build_upsample_layers(self, hidden_features):
#         # Create upsample layers as reusable components
#         return nn.Sequential(
#             nn.ConvTranspose2d(self.in_channels, hidden_features, kernel_size=2, stride=2),
#             nn.Sequential(
#                 nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1),
#                 nn.BatchNorm2d(hidden_features),
#                 nn.ReLU()
...
#     def _build_downsample_layer(self, hidden_features):
#         return nn.Sequential(
#             nn.Conv2d(self.in_channels, hidden_features, kernel_size=2, stride=2),
#             nn.BatchNorm2d(hidden_features),
#             nn.ReLU()
#         )
...
class Connect(nn.Module):
    def __init__(self, in_channels=768, out_channels=128, ratio=1):
        ...
        self.c1 = UpSample(in_channels=self.in_channels, out_channels=128, ratio=3, up_step=2)
        self.c2 = UpSample(in_channels=self.in_channels, out_channels=256, ratio=3, up_step=1)
        self.c3 = UpSample(in_channels=self.in_channels, out_channels=512, ratio=3, up_step=0)
        self.c4 = UpSample(in_channels=self.in_channels, out_channels=1024, ratio=3, up_step='downsample')

    def forward(self, inputs):
        # x1 = self.to_2D(inputs[0])
        # x2 = self.to_2D(inputs[1])
        # x3 = self.to_2D(inputs[2])
        # x4 = self.to_2D(inputs[3])
        print(f'c1_: {inputs[0].size()}')
        print(f'c1_: {inputs[1].size()}')
        print(f'c1_: {inputs[2].size()}')
        print(f'c1_: {inputs[3].size()}')
        c1 = self.c1(inputs[0])
        print(f'c1_: {c1.size()}')
        c2 = self.c2(inputs[1])
        print(f'c2_: {c2.size()}')
...
# class Connect(nn.Module):
#     def __init__(self, in_channels=768, out_channels=128, patch_size=16, ratio=1, up_steps=None):
#         """
#         Args:
#             in_channels: Input channels.
#             out_channels: Output channels.
#             patch_size: Size of the patch (not used directly here).
#             ratio: Downsampling ratio.
#             up_steps: List of upsample configurations, e.g., [2, 1, 0, "downsample"].
#         """
#         super(Connect, self).__init__()
#         self.in_channels = in_channels
#         self.out_channels = out_channels
...
#         if up_steps is None:
#             up_steps = [2, 1, 0, "downsample"]

#         self.upsample_blocks = nn.ModuleList([
#             UpSample(in_channels, out_channels, ratio, up_step=step) for step in up_steps
#         ])

#     def forward(self, inputs):
#         """
#         Args:
#             inputs: A list of feature maps. Should match the number of upsample blocks.
#         """
#         assert len(inputs) == len(self.upsample_blocks), \
#             f"Expected {len(self.upsample_blocks)} inputs, but got {len(inputs)}"
#         outputs = [block(x) for block, x in zip(self.upsample_blocks, inputs)]
#         return outputs
...
class WT_ML_ViT(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(WT_ML_ViT, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(WT_ML_ViT, self).init_weights()
